...
void Preprocess::mid_handler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
  std::cout << "entry mid" << std::endl;
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
...
void Preprocess::velodyne_handler(
  ...
  yaw_cali = yaw_cali > 300.0
                 ? (yaw_cali - 360.0)
                 : yaw_cali < -300.0 ? (yaw_cali += 360.0) : yaw_cali;
  if (yaw_cali < 15.0)
