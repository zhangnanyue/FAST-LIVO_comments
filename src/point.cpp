...
Point::Point(const Vector3d &pos)
    : id_(point_counter_++), pos_(pos), normal_set_(false), n_obs_(0),
      last_published_ts_(0), last_projected_kf_id_(-1),
      // type_(TYPE_UNKNOWN),
      n_failed_reproj_(0), n_succeeded_reproj_(0), last_structure_optim_(0),
      have_scaled(false) {}

Point::Point(const Vector3d &pos, FeaturePtr ftr)
    : id_(point_counter_++), pos_(pos), normal_set_(false), n_obs_(1),
      last_published_ts_(0), last_projected_kf_id_(-1),
      // type_(TYPE_UNKNOWN),
      n_failed_reproj_(0), n_succeeded_reproj_(0), last_structure_optim_(0),
      have_scaled(false) {
...
bool Point::getClosePose(const FramePtr &new_frame, FeaturePtr &ftr) const {
  ...
  for (auto it = obs_.begin(), ite = obs_.end(); it != ite; ++it) {
    SE3 delta_pose =
        (*it)->T_f_w_ * new_frame->T_f_w_.inverse(); // dir.normalize();
    double delta_theta =
  ...
  if (min_cos_angle >
      2.0) // assume that observations larger than 60° are useless 0.5
  {
...
// 获取历史观测夹角最小的的Feature
// 1.如果观测次数为0,则直接退出，及没有观测
// 2.获取当前帧的相机位置-该点的位置，即获得在相机坐标系下的观测向量obs_dir，并归一化
// 3.循环所有的历史观测
//   3.1.计算每个观测点的在相机坐标系下的观测向量dir，并归一化
//   3.2.将当前帧的向量obs_dir投影至dir上，获取其夹角
//   3.3.找到观测角度最小的那个历史Feature，但是如果该角度角度超过60,也舍弃
bool Point::getCloseViewObs(const Vector3d &framepos, FeaturePtr &ftr,
                            const Vector2d &cur_px) const {
  // TODO: get frame with same point of view AND same pyramid level!
  if (obs_.size() <= 0)
    return false;

  // 当前帧的相机位置-该点的位置，即获得在相机坐标系下的观测向量obs_dir
  ...
  // 计算每个观测点的在相机坐标系下的观测向量dir，并归一化
  // 并将当前帧的向量obs_dir投影至dir上，获取其夹角
  // 找到观测角度最小的那个历史Feature，但是如果该角度角度超过60,也舍弃
  for (auto it = obs_.begin(), ite = obs_.end(); it != ite; ++it) {
...
bool Point::getCloseViewObs_test(const Vector3d &framepos, FeaturePtr &ftr,
                                 const Vector2d &cur_px,
...
void Point::getFurthestViewObs(const Vector3d &framepos,
                               FeaturePtr &ftr) const {
  ...
  // {
  //   Vector3d dir((*it)->T_f_w_.inverse().translation() - pos_);
  //   dir.normalize(); double cos_angle = obs_dir.dot(dir); if(cos_angle <
  //   max_cos_angle)
  //   {
