...
// Hash value
namespace std {
template <> struct hash<VOXEL_KEY> {
...
struct LidarMeasureGroup {
  ...
  void debug_show() {
    ...
    std::cout << "last_update_time:" << setprecision(20)
              << this->last_update_time << endl;
    std::cout << "lidar_beg_time:" << setprecision(20) << this->lidar_beg_time
              << endl;
    for (auto it = this->measures.begin(); it != this->measures.end();
         ++it, ++i) {
      std::cout << "In " << i << " measures: ";
      for (auto it_meas = it->imu.begin(); it_meas != it->imu.end();
           ++it_meas) {
        std::cout << setprecision(20)
                  << (*it_meas)->header.stamp.toSec() - this->lidar_beg_time
                  << " ";
      }
      std::cout << "img_time:" << setprecision(20) << it->img_offset_time
                << endl;
    }
...
struct SparseMap {
  ...
  void delete_points() {
    ...
    if (size > 1000) {
      ...
      this->xyz_ref.erase(this->xyz_ref.begin(),
                          this->xyz_ref.begin() + size - 1000);
    }
  }
  ...
  void update_point(int ind, float *newP) {
    ...
    float v = fabs(2 * (P[6] - P[4]) + P[2] - P[0] + P[10] - P[8]) +
              fabs(2 * (P[9] - P[1]) + P[10] - P[2] + P[8] - P[0]);
  }
};
