...
public:
  ...
  V3D Pli, Pci, Pcw;
  int *align_flag;
  // 网格里面存储的3D点的类型，如果没有点是UNKNOWN，地图点是TYPE_MAP，
  int *grid_num;
  int *map_index;
  // 网格中深度最近的点的深度，是在for循环遍历中不断变小的值
  float *map_dist;
  // 网格中角点的 shiTomasiScore 得分的最大值，是在for循环遍历中不断变大的值
  float *map_value;
  float *patch_cache;
  ...
  void createPatchFromPatchWithBorder(float *patch_with_border,
                                      float *patch_ref);
  void getWarpMatrixAffine(const vk::AbstractCamera &cam,
                           ...
                           Matrix2d &A_cur_ref);
  bool align2D(const cv::Mat &cur_img, float *ref_patch_with_border,
               float *ref_patch, const int n_iter, Vector2d &cur_px_estimate,
               int index);
  ...
  // 当前帧图像用到的子地图的3D点，是网格中深度最近的那个点，每次都会重新构造
  // 本次找到的视觉地图中的点
  ...
  FramePtr last_kf_;
  Map map_;
  enum Stage { STAGE_FIRST_FRAME, STAGE_DEFAULT_FRAME };
  Stage stage_;
